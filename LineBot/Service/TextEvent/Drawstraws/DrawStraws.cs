...
namespace LineBot.Domain.TextEvent
{
    public class DrawStraws : BaseResponse, ITextEvent
    {
        ...
        private void RandomLots()
        {
            ...
            {
                int tempPosition = rand.Next(0, Lots.Count);
                var temp = Lots[i];
                Lots[i] = Lots[tempPosition];
                Lots[tempPosition] = temp;
            }
        }

        public List<(int count, string type, string describe)> LotTypes()
        {
            return new List<(int count, string type, string describe)>
                    {
                        (3, "大吉","萬人之上，老鼠之下。"),
